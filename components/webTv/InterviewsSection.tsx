...
interface Props {
  interviews: DocVideo[];
  /** Layouts genuinely differ (stacked list vs. 3-col grid, different badge placement) —
   * kept as two JSX blocks in one component rather than a single responsive tree. Omit to let
   * both render and toggle via `lg:hidden`/`hidden lg:block`; pass explicitly when the caller
   * already branches mobile/desktop at a higher level (avoids mounting both). */
  variant?: "mobile" | "desktop";
}

function InterviewsMobile({ interviews }: { interviews: DocVideo[] }) {
  return (
    <section className="flex flex-col gap-3 px-4 pb-4">
      <div className="flex items-center justify-between pt-2">
        <div className="flex items-center gap-1.5">
          ...
          <h2 className="text-white text-base font-bold tracking-tight">Interviews</h2>
        </div>
        <a className="text-primary text-xs font-bold" href="/interviews">
          Voir tout
        </a>
      </div>
          ...
          >
            ...
            <div className="flex flex-col justify-center py-2 pr-3 gap-0.5 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-gray-500 text-[9px]">{interview.date}</span>
              </div>
            </div>
          </Link>
        ))}
...
function InterviewsDesktop({ interviews }: { interviews: DocVideo[] }) {
  return (
        ...
        {interviews.map((interview) => (
          ...
          >
            ...
            <div className="flex flex-col gap-2 p-4 flex-1">
              ...
              <span className="text-gray-500 text-xs mt-auto">{interview.date}</span>
            </div>
          </Link>
        ))}
      </div>
...
export default function InterviewsSection({ interviews, variant }: Props) {
  if (variant === "mobile") return <InterviewsMobile interviews={interviews} />;
  if (variant === "desktop") return <InterviewsDesktop interviews={interviews} />;

  return (
    <>
      <div className="lg:hidden">
        <InterviewsMobile interviews={interviews} />
      </div>
      <div className="hidden lg:block">
        <InterviewsDesktop interviews={interviews} />
      </div>
    </>
  );
}
