...
export default function FreestylesSectionDesktop({ videos }: { videos: FreestyleVideo[] }) {
  return (
    <section className="flex flex-col gap-4">
      ...
      {/* Masonry 4 colonnes — columns CSS */}
      <div className="columns-4 gap-4 space-y-4">
        {videos.map((video) => (
          <Link
            key={video.id}
            href={video.href || "#"}
            className="relative break-inside-avoid rounded-xl overflow-hidden bg-surface-dark group ring-1 ring-white/5 cursor-pointer"
          >
            {video.isNew && (
