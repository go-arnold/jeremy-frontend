...
| 2026-07-26 | Radio — `/api/v1/radio/current/` ne reflète pas un vrai direct | Confirmé en direct : l'endpoint a renvoyé un programme avec `status: "upcoming"` (pas `"live"`), alors qu'aucun programme n'était réellement en cours — le frontend faisait pourtant confiance à cet endpoint sans vérifier le `status` | Soit corriger l'endpoint pour ne renvoyer un programme que s'il est réellement `"live"` (404 sinon, comme `/emissions/live/`), soit documenter que le frontend doit toujours vérifier `status` lui-même | 🟡 en discussion — le frontend calcule maintenant lui-même le programme à mettre en avant à partir de `/radio/program/` (statut + jour/heure), sans plus faire confiance à `/radio/current/` |
| 2026-07-26 | Radio — statuts `"ended"` incomplets sur la grille de la semaine | Sur les 63 créneaux de la semaine en cours, un seul avait `status: "ended"` alors que la plupart des créneaux déjà passés cette semaine (avant aujourd'hui) auraient dû l'être aussi | Vérifier que le passage `upcoming → live → ended` se fait bien automatiquement pour chaque créneau une fois son horaire passé (tâche planifiée manquante ?) | 🟡 en discussion — n'empêche pas le frontend de fonctionner (il prend le plus récent `"ended"` disponible, quel que soit leur nombre), mais fausse la rediffusion si des créneaux passés restent bloqués sur `"upcoming"` |
| 2026-07-26 | Émissions — plusieurs émissions `status: "live"` simultanément | Confirmé en direct : 3 émissions avaient `status: "live"` en même temps sur `/api/v1/emissions/` ; `/api/v1/emissions/live/` n'en renvoie qu'une seule sans règle de départage documentée | Documenter la règle de départage de `/emissions/live/`, ou exposer un `live_started_at` (comme `MusicLiveSession`) pour permettre au frontend de choisir lui-même la plus récente de façon fiable | 🟡 en discussion — le frontend ne fait plus confiance à `/emissions/live/` seul : il trie désormais `/emissions/?ordering=-scheduled_at` et prend la première `"live"` (best-effort sans `live_started_at` réel) |
