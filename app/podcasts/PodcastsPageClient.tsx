...
import type { FeaturedEpisode, SelectionEpisode, RecentEpisode, LatestPickEpisode, PodcastCategory } from "@/types/podcasts";
import type { ApiEpisode } from "@/lib/api-types";

type Episode = ReturnType<typeof mapApiPodcastToEpisode>;
...
export default function PodcastsPageClient({
  ...
  const showEmptyState = !loading && episodes.length === 0;
      ...
      {showEmptyState ? (
        ...
        <>
          ...
          <main className="lg:hidden pb-14">
            {/* PodcastCategory is a closed design-time union, but real categories come from the
                API at runtime — same widening the original code did with `as any`, just narrower. */}
            <PodcastCategoryFilter categories={categories as PodcastCategory[]} onChange={handleCategoryChange} />
            <FeaturedEpisodeCard episode={featuredEpisode as unknown as FeaturedEpisode} />
            <SelectionEpisodeCard episode={selectionEpisode as unknown as SelectionEpisode} />
            <RecentEpisodesList episodes={recentEpisodes as unknown as RecentEpisode[]} />
            <LatestPicksList episodes={latestPickEpisodes as unknown as LatestPickEpisode[]} />
            <VoirPlusPagination
              onLoadMore={loadMore}
              hasMore={hasMore}
            ...
            <div className="max-w-7xl mx-auto px-8 w-full flex flex-col gap-12">
              <div className="grid grid-cols-[11fr_9fr] gap-8 items-start">
                <div className="flex flex-col gap-6">
                  <FeaturedEpisodeDesktop episode={featuredEpisode as unknown as FeaturedEpisode} />
                  <SelectionEpisodeDesktop episode={selectionEpisode as unknown as SelectionEpisode} />
                </div>
                <div className="flex flex-col gap-5">
                  ...
                  <div className="flex flex-col gap-3">
                    {recentEpisodes.map((ep) => (
                      <RecentEpisodeRowDesktop key={ep.id} episode={ep as unknown as RecentEpisode} />
                    ))}
                  </div>
                </div>
              </div>
              ...
              <section>
                ...
                <div className="grid grid-cols-3 gap-5">
                  {latestPickEpisodes.map((ep) => (
                    <LatestPickCardDesktop key={ep.id} episode={ep as unknown as LatestPickEpisode} />
                  ))}
                </div>
              </section>
