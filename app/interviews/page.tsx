...
async function getInterviews() {
  try {
    const res = await apiFetch<PaginatedResponse<ApiVideo>>(
      `/api/v1/webtv/videos/?category=interviews&page_size=50`,
...
function InterviewCard({ video }: { video: WebTVVideo }) {
  return (
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-950 to-black pt-32 pb-20">
      <div className="container mx-auto px-4">
        ...
        <h1 className="text-5xl font-bold text-white mb-4">Interviews</h1>
        <p className="text-white/60 mb-12">Rencontrez les voix et visages du Kivu.</p>

        {data.results?.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {data.results.map((video) => (
              <Suspense key={video.id} fallback={<div className="bg-slate-800 rounded-xl h-48 animate-pulse" />}>
                <InterviewCard video={video} />
              </Suspense>
            ))}
          </div>
        ) : (
          <div className="text-center py-20">
            <p className="text-white/60">Aucune interview disponible</p>
          </div>
        )}
      </div>
    </main>
