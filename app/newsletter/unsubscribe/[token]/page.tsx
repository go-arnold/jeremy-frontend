...
export default function NewsletterUnsubscribePage() {
  ...
  const [message, setMessage] = useState(
    token ? "Traitement de votre désabonnement..." : "Lien de désabonnement invalide."
  );
  ...
  useEffect(() => {
    ...
    unsubscribeFromNewsletter(token)
      .then((res) => {
        setStatus("success");
        setMessage(res.detail || "Vous avez été désabonné de la newsletter.");
      })
      .catch((error) => {
        setStatus("error");
        setMessage(
          error instanceof Error ? error.message : "Le désabonnement a échoué. Le lien est peut-être expiré."
        );
      });
  }, [token]);
      ...
      <div className="w-full max-w-sm relative z-10 bg-[#12223c]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 text-center shadow-2xl">
        ...
        >
          <span className="material-symbols-outlined text-3xl">
            {status === "success" ? "unsubscribe" : status === "error" ? "error" : "hourglass_empty"}
          </span>
        </div>

        <h1 className="text-xl font-black text-[#F0EDE8] mb-3 uppercase tracking-tight">
          Newsletter <span className="text-primary">Désabonnement</span>
        </h1>
